...
export const salesOrderStatusClmn = (
  ...
  setPartnerMngData: React.Dispatch<
  ...
  // },
  {
    ...
    render: (_, record: salesOrderWorkSheetType) => (
      <div className="w-full v-h-venter">
        {record?.worksheet?.specModel?.prdMngNo
          ? record?.worksheet?.specModel?.prdMngNo
          : "사양미확정"}
      </div>
    ),
  ...
  },
  {
    ...
    render: (_, record: salesOrderWorkSheetType) => (
      <div className="w-full h-full v-h-center">
        {record?.worksheet?.specModel?.pcsW ||
        record?.worksheet?.specModel?.pcsL
          ? (record?.worksheet?.specModel?.pcsW ?? "") +
            " * " +
            (record?.worksheet?.specModel?.pcsL ?? "")
          : ""}
  ...
  },
  {
    ...
    render: (_, record: salesOrderWorkSheetType) => (
      <div className="w-full h-full v-h-center">
        {record?.worksheet?.specModel?.pnlW ||
        record?.worksheet?.specModel?.pnlL
          ? (record?.worksheet?.specModel?.pnlW ?? "") +
            " * " +
            (record?.worksheet?.specModel?.pnlL ?? "")
          : ""}
...
export const salesUserOrderClmn = (
  ...
  },
  {
    title: "최단 납기일",
    width: 120,
    dataIndex: "shortOrderPrdDueDt",
    key: "shortOrderPrdDueDt",
    align: "center",
  },
  {
    title: "모델상태",
        ...
        <div
          ...
          onClick={() => {
            if (value.includes("new")) {
              setNewProducts(
                newProducts.filter(
                  (f: salesOrderProcuctCUType) => f.id !== value
                )
              );
            } else {
