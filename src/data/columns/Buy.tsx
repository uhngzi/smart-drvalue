  ...
  },
  {
    ...
    render: (_, record, index) => (
      <GlobalMemo
        key={index}
        id={record.id ?? ""}
        entityName="RnTenantCbizProcessVendorPriceEntity"
        entityRelation={{
          RnTenantCbizBizPartnerEntity: true,
          RnTenantCbizProcessGroupEntity: true,
          RnTenantCbizProcessEntity: true,
        }}
      />
    ),
  },
...
export const BuyCostOutPriceClmn = (
...
): CustomColumn[] => [
  {
    ...
    render: (value: string, record: processVendorPriceRType) => (
      <div
        className="v-h-center w-full h-full"
        style={{
          background:
            select === record.id
      ...
      >
        <input
          ...
          name={record.process.id}
          checked={select === record.id ? true : false}
          onChange={() => {
            setSelectPrice([
  ...
  },
  {
    ...
    render: (value, record) => (
      <div
        className="v-h-center w-full h-full"
        style={{ background: select === record.id ? "#F0F5FF" : "" }}
      >
        {value}
      </div>
    ),
...
export const BuyCostOutStatusClmn = (
  ...
  },
  {
    ...
    render: (value: 0 | 1 | boolean, record: buyCostOutType) => (
      <div className="w-full h-full v-h-center">
        {value === 0 || value === false ? (
          <FullChip state="yellow" label="등록전" />
        ) : (
  ...
  },
  {
    ...
    render: (_, record, index) => (
      <GlobalMemo
        key={index}
        id={record.id ?? ""}
        entityName="RnTenantCbizRequestMaterialEntity"
        entityRelation={{
...
export const BuyOrderMtPriceClmn = (
  ...
  },
  {
    ...
    render: (value: string, record: materialPriceType) => (
      <div
        style={{ background: record.id === item.mtPriceIdx ? "#F0F5FF" : "" }}
...
export const BuyOrderMtClmn = (
  ...
  setSelectMtIdx: React.Dispatch<
    SetStateAction<{ mtIdx: string; orderId: string; orderNo: number } | null>
  >,
...
): CustomColumn[] => [
  {
    ...
    render: (_: any, record: buyOrderDetailType, index: number) => (
      <div className="w-full h-full v-h-center">
        <Tooltip
          title={
            selectMtIdx?.orderId === record.id
              ? "현재 단가 목록에 보여지고 있는 원자재예요"
              : "클릭 시 해당 발주 품목의 원자재 단가 목록을 선택할 수 있어요"
          }
        >
          <div
          ...
          <div className="w-1/2">
            {record.materialGrpIdx === "직접입력" ? (
              <AntdInput
                value={record.mtNm}
                onChange={(e) =>
                  handleDataChange(record.id ?? "", "mtNm", e.target.value)
                }
                placeholder="원자재명 입력"
                memoView
              />
              ...
              <AntdSelect
                options={(
                  mtGrp.find((f) => f.id === record.materialGrpIdx)
                    ?.materials ?? []
                ).map((item) => ({
                  value: item.id,
                  label: item.mtNm ?? "",
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderUnit", e.target.value)
            }
            placeholder="단위 입력"
            memoView
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderTxtur", e.target.value)
            }
            placeholder="재질 입력"
            memoView
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderSizeW", e.target.value)
            }
            type="number"
            placeholder="W 입력"
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderSizeH", e.target.value)
            }
            type="number"
            placeholder="H 입력"
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderThk", e.target.value)
            }
            type="number"
            placeholder="두께 입력"
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderWeight", e.target.value)
            }
            type="number"
            placeholder="무게 입력"
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderQty", e.target.value)
            }
            type="number"
            placeholder="수량 입력"
          />
      ...
      return (
        <div className="w-full h-center px-10">
          <AntdInput
            ...
            onChange={(e) =>
              handleDataChange(record.id ?? "", "mtOrderAmount", e.target.value)
            }
            type="number"
            placeholder="금액 입력"
          />
  ...
  },
  {
    ...
    render: (_, __, index: number) => (
      ...
      >
        <p className="w-16 h-16 text-[red]">
          <Trash />
        </p>
      </div>
    ),
  ...
  },
  {
    ...
    render: (_, record) => (
      <div>
        <AntdInput
          value={
            badCnt.find((f) => f.badId === record.id && f.mtId === item.id)?.cnt
          }
          onChange={(e) => {
            setBadCnt([
              ...badCnt.filter(
                (f) => !(f.badId === record.id && f.mtId === item.id)
              ),
              {
                badNm: record.badNm ?? "",
                badId: record.id ?? "",
...
export const BuyOrderMtViewClmn = (): CustomColumn[] => [
  {
    title: "순서",
    width: 50,
    dataIndex: "order",
