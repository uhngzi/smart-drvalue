...
export async function onTreeDelete(
...
export function updateTreeDatas(
  ...
  const filteredAddList = addList
    ...
    .filter(
      (addItem) =>
        !deleteList.some((deleteItem) => deleteItem.id === addItem.id)
    );
  const filteredEditList = editList.filter(
    (editItem) =>
      !deleteList.some((deleteItem) => deleteItem.id === editItem.id)
  );
  ...
  const updatedAddList = filteredAddList.map((addItem) => {
    const editItem = filteredEditList.find(
      (editItem) => editItem.id === addItem.id
    );
    if (editItem) {
  ...
  const updatedDeleteList = deleteList.filter(
    (deleteItem) => !deleteItem.id.includes("temp")
  );
