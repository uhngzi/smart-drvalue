...
const WKStatusWIPPage: {
  ...
  const {
    ...
    queryKey: [
      "worksheet/production-status/process-status/by-type/jsxcrud/many",
      pagination,
      procTypeIdx,
      sQueryJson,
    ],
  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      const arr = (queryData?.data?.data ?? []).map((item: wkPlanWaitType) => ({
        ...
        wkLatestDtm: item.wkLatestDtm
          ? dayjs(item.wkLatestDtm).format("YYYY-MM-DD")
          : null,
        make:
