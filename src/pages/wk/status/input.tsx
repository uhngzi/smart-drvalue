...
const WKStatusProcPage: {
  ...
  const handlePageMenuClick = (key: number) => {
    const clmn = WKStatusInClmn(totalData, pagination, setPartnerData).map(
      (item) => ({
  ...
  const { data: queryVendorData } = useQuery({
    queryKey: [
      "worksheet/production-status/input-status/search-vender/default/vender-list",
    ],
    queryFn: async () => {
  ...
  const {
    ...
    queryKey: [
      "worksheet/production-status/input-status/jsxcrud/many",
      pagination,
      selectVendor,
      sQueryJson,
    ],
      ...
      return getAPI(
        ...
        },
        {
          ...
          anykeys: {
            ...
            procVenderIdx: selectVendor?.vendorId,
          },
        }
      );
    },
      ...
      <div className="w-[7%] pt-15">
        ...
        <div className="w-full max-h-[700px] overflow-y-auto mt-10 flex flex-col gap-10">
          <div
            ...
            style={
              selectVendor?.vendorId === undefined
                ? { border: 0, background: "#F5F6FA" }
                : {}
            }
            onClick={() => {
          ...
          {vendorList.map((item, index) => (
            <div
              ...
              className="h-50 v-between-h-center border-1 border-line rounded-14 p-10 cursor-pointer"
              style={
                selectVendor?.vendorId === item.vendorId
                  ? { border: 0, background: "#F5F6FA" }
                  : {}
              }
              onClick={() => {
