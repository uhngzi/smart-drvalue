...
const SayangSampleAddPage: React.FC & {
    ...
    if (!isLoading && queryData?.resultCode === "OK_0000") {
      ...
      const models = rdata?.specModels?.map(
        (model: specModelType, index: number) => ({
          ...
          index: index + 1,
          pcsValue:
            model.pcsValue && model.pcsValue > 0
              ? model.pcsValue
              : model.modelMatch?.orderModel?.orderPrdCnt,
        })
      );
      setDetailData({
        ...rdata,
  ...
  const { data: prdGrpQueryData } = useQuery<apiGetResponseType, Error>({
  ...
  // 제품군 그룹 선택값
  const [selectPrdGrp, setSelectPrdGrp] =
  ...
  const [selectedVendors, setSelectedVendors] = useState<
    { pid: string; vid: string; vname: string }[]
  >([]);
    ...
    if (!open) {
      ...
      if ((detailData.specPrdGroupPrcs ?? []).length > 0) {
        ...
        const prc = rdata?.find(
          (f) =>
            f.id === detailData.specPrdGroupPrcs?.[0]?.productLinesGroup?.id
        );
  ...
  const [resultType, setResultType] = useState<
    "cf" | "error" | "del" | "processClose" | "fileClose" | ""
  >("");
    ...
    try {
      if (deleted) {
        ...
        if (result.resultCode === "OK_0000") {
          ...
          if (
            detailData.specModels?.length &&
            detailData.specModels?.length < 2
          ) {
  ...
  return (
    <>
      <div className="px-30 min-h-70 !h-70 v-between-h-center w-full">
        <p className="text-18 font-[500]">사양 등록</p>
        <p
      ...
      >
        <div className="w-full pr-30 flex flex-col gap-40">
          ...
          {!detailDataLoading && (
            <>
              {modelTabOpen && (
                <Popup className="overflow-auto !py-30">
            ...
            />
              ...
              <div className="v-between-h-center px-30">
                {detailData?.specPrdGroupPrcs &&
                detailData?.specPrdGroupPrcs?.length > 0 ? (
                  <Button
                  ...
                  <div className="v-h-center gap-5">
                    ...
                    <FullOkButtonSmall
                      ...
                      click={() => {
                        if (
                          detailData.specPrdGroupPrcs &&
                          detailData.specPrdGroupPrcs?.length < 1
                        ) {
          ...
          <AntdAlertModal
            ...
            title={
              resultType === "cf"
                ...
                ? "공정 지정을 하지 않고 닫으시겠습니까?"
                : resultType === "fileClose"
                ? "변경된 설계도면을 저장하지 않고 닫으시겠습니까?"
                : ""
            }
            contents={
              resultType === "cf" ? (
                <div className="h-40">사양 확정에 성공하였습니다.</div>
              ) : resultType === "error" ? (
                <div className="h-40">{resultMsg}</div>
              ) : resultType === "del" ? (
                <div className="h-40">
                  해당 모델의 사양 등록을 취소하시겠습니까?
                </div>
              ) : resultType === "processClose" ? (
                <div className="h-40">
                  저장되지 않은 공정이 있습니다.
            ...
            type={
              resultType === "cf"
                ...
                : resultType === "del"
                ? "warning"
                : resultType === "processClose" || resultType === "fileClose"
                ? "warning"
                : "success"
            }
            onOk={() => {
...
SayangSampleAddPage.layout = (page: React.ReactNode) => (
  <MainPageLayout menuTitle="사양 등록" head={false} modal={true}>
    {page}
  </MainPageLayout>
);
