...
const BuyMtListPage: React.FC & {
  ...
  useEffect(() => {
    setAddModalInfoList(MOCK.mtItems.CUDPopItems.map(v => v.name === 'unitType' ? {...v, option: unitSelectList} : v))
  },[unitSelectList])
  ...
  const { refetch: groupRefetch } = useQuery<apiGetResponseType, Error>({
    ...
    queryFn: async () => {
      ...
      if (result.resultCode === 'OK_0000') {
        ...
        console.log(addList)
        setAddModalInfoList((prev:any) => prev.map((d:any) => d.name === 'materialGroupId' ? {...d, option: addList} : d));
