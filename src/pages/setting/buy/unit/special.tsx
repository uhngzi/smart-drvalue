...
const BuyUnitSpecialListPage: React.FC & {
  ...
  const [ newOpen, setNewOpen ] = useState<boolean>(false);
    //등록 모달창 데이터
  const [ newData, setNewData ] = useState<unitSpecialCUType>(newUnitSpecialCUType);
  const [addModalInfoList, setAddModalInfoList] = useState<any[]>(MOCK.unitSpecialItems.CUDPopItems);
  ...
  useEffect(() => {
    if ((!unitSelectList || unitSelectList.length < 1) || (!memoDataProcessList || memoDataProcessList.length < 1)) return;
