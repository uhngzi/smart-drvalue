...
useEffect(() => {
  if (
    materialOptions.length > 0 || copperList.length > 0 || materialEpoxy.length > 0 || materialCode.length > 0 || copperListCopThk.length > 0) 
    {
      ...
      if (item.optionSource === 'materialOptions' && materialOptions.length > 0) {
        const filteredMaterialOptions = newData?.lamDtlTypeEm
          ? materialOptions.filter(opt =>
              dataGroup.find(d =>
                d.id === opt.value && d.lamDtlTypeEm === newData.lamDtlTypeEm
              )
            )
          : materialOptions;
...
}, [materialOptions, copperList, materialEpoxy, materialCode, copperListCopThk, newData?.lamDtlTypeEm, dataGroup ]);
        ...
        <div className="v-between-h-center pb-20">
          <div className="flex gap-10">
            ...
            <Radio.Group value={type ? type : ""} size="small" className="custom-radio-group">
              <Radio.Button value="" onClick={() => {console.log("유형: "); setType(''); }}>전체</Radio.Button>
              <Radio.Button value="cf" onClick={() => {console.log("유형: CF");setType('cf');}}>C/F</Radio.Button>
              <Radio.Button value="pp" onClick={() => {console.log("유형: PP");setType('pp');}}>P/P</Radio.Button>
              <Radio.Button value="ccl" onClick={() => {console.log("유형: CCL");setType('ccl');}}>CCL</Radio.Button>
            </Radio.Group>
          </div>
          <div
