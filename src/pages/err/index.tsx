...
const ErrBoardPage: React.FC & {
  ...
  const { refetch } = useQuery<apiGetResponseType, Error>({
    queryKey: [
      "require-tenant-key/board/bugfix/jsxcrud/many",
      pagination,
      sQueryJson,
    ],
    queryFn: async () => {
  ...
  const { refetch: commentsRefetch } = useQuery<apiGetResponseType, Error>({
    queryKey: [
      "require-tenant-key/board/bugfix/comment/default/many/by-board-id",
      select?.id,
      paginationCmt,
    ],
    queryFn: async () => {
      ...
      if (result.resultCode === "OK_0000") {
        const arr = ((result.data?.comments as errCommentType[]) ?? []).map(
          (cmt) => ({
            ...cmt,
            childShow: false,
          })
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (result.resultCode === "OK_0000") {
        ...
        setSelect({
          ...entity,
          attachmentFiles: JSON.parse(attachmentFilesStr),
        });
        console.log(entity);
          ...
          <AntdTableEdit
            columns={[
              ...
              },
              {
                ...
                cellAlign: "left",
                tooltip:
                  "내용을 클릭하시면 세부 내용 확인 및 상태 변경, 댓글 작성을 할 수 있습니다",
                render: (value, record) => (
                  <div
                    ...
                    onClick={() => {
                      setSelect(record);
                      setFileIdList(
                        (record as errBoardType).attachmentFiles ?? []
                      );
                      setOpen(true);
                    }}
                  >
              ...
              <div className="w-full h-full min-h-100 bg-white rounded-14 p-20 flex flex-col gap-20">
                ...
                <div className="w-full h-full flex items-start gap-20">
                  <div className="w-1/3 h-full flex flex-col gap-10">
                    ...
                    {select?.id &&
                      select?.attachmentFiles &&
                      select?.attachmentFiles?.length > 0 &&
                      select?.attachmentFiles?.map((fileId, index) => (
                        <div className="h-center" key={index}>
                          ...
                          <div
                            ...
                            onClick={() =>
                              downloadFileByObjectName(fileId, fileList[index])
                            }
                          >
                            {fileList[index]?.name}
                          </div>
                        ...
                        >
                          ...
                          {myCmt?.parentId === cmt?.id && (
                            <div className="h-center gap-10 pl-30">
                              ...
                              <Button
                                onClick={() => {
                                  setMyCmt({ ...myCmt, parentId: undefined });
                                }}
                                className="border-point1"
                              >
                                <CloseCircleTwoTone />
                              </Button>
                              <Button
                          ...
                          {cmt?.childComments &&
                            cmt?.childShow &&
                            cmt?.childComments.map((child) => (
                              <div
                                className="h-center gap-10 pl-10"
                                key={child.id}
                              >
                                <CommentOutlined />
                                <div className="max-w-100 min-w-100 border-r-2 border-r-black">
...
ErrBoardPage.layout = (page: React.ReactNode) => (
  <MainPageLayout menuTitle="오류사항 게시판">{page}</MainPageLayout>
);
