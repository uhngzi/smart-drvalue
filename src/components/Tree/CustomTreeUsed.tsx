              ...
              {isChild && (
                ...
                >
                  {item.children?.map((child) => (
                    <div key={child.id} className={`w-full h-40 h-center gap-10 pl-20 cursor-pointer hover:bg-[#f3f6f7] ${selectId === child.id ? '!bg-[#f3faff]' : ''}`} onClick={() => handleSelect(child)}>
                      ...
                      <div className="relative flex-1 flex v-between-h-center">
                        ...
                        { isCheck ? <Checkbox onChange={(e) => checkChange?.(e, checkedData?.find(chk => chk.checkId === child.id)?.matchId || null)} value={child.id} checked={checkedData?.some(chk => chk.checkId === child.id)}/> : <></> }
