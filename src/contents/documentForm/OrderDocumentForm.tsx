...
const OrderDocumentForm: React.FC<Props> = ({ id }) => {
        ...
        <table className="h-[25px] table-auto w-full border-t border-[#D9D9D9]">
          ...
          <tbody className="mb-0 font-[Spoqa Han Sans Neo] text-[10px] font-style:normal bg-[rgba(238,238,238,0.5)]">
            <tr className="border-b border-[#D9D9D9] h-[25px] text-center align-middle whitespace-nowrap">
              ...
              <td className="border-r border-[#D9D9D9] w-[335px] h-[25px] px-[8px] ">
                품명
              </td>
              <td className="border-r border-[#D9D9D9] w-[40px] h-[25px] px-[8px] ">
                수량
          ...
          <tbody className="font-[Spoqa Han Sans Neo] text-[10px] font-style:normal text-center align-middle">
            ...
            {order?.detailInfo?.details &&
              order?.detailInfo?.details.map((item, index) => (
                <tr
                  key={index}
                  className="border-b border-[#D9D9D9] h-[30px] text-[#000000A6]"
                >
                  ...
                  <td className="border-r border-[#D9D9D9] px-[8px]">
                    {(item.mtOrderQty ?? 0).toLocaleString()}
                  </td>
                  <td className="border-r border-[#D9D9D9] px-[8px]">
                    {(item.mtOrderInputPrice ?? 0).toLocaleString()}
                  </td>
                  <td className="px-[8px]">
            ...
            {/* 빈 줄 */}
            {Array.from(
              { length: Math.max(0, 16 - sample.length - 1) },
              (_, index) => (
                <tr
                  key={`empty-${index}`}
                ...
                >
                  {Array.from({ length: 5 }).map((_, colIdx) => (
                    <td
                      key={colIdx}
                      className={`px-[8px] ${
                        colIdx !== 4 ? "border-r border-[#D9D9D9]" : ""
                      }`}
                    >
                      &nbsp;
                    </td>
          ...
          <tbody className="mb-0 font-[Spoqa Han Sans Neo] text-[10px] font-style:normal text-[#000000A6]">
            <tr className="border-b border-[#D9D9D9] h-[25px] text-center align-middle whitespace-nowrap">
              <td className="border-r border-[#D9D9D9] w-[40px] h-[96px] px-[8px] ">
                비고
              </td>
              <td
                colSpan={4}
                className="max-w-[75px] h-[96px] pl-[8px] pt-[5px] whitespace-nowrap pb-[5px]"
              >
                -
              </td>
            </tr>
