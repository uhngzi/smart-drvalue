...
const LaminationContents: React.FC<Props> = ({
  ...
  // ------------ 적층구조 구성요소 세팅 ----------- 시작
  const [baseLamination, setBaseLamination] = useState<
    Array<laminationSourceList>
  >([]);
  const {
    data: baseLaminationData,
    isLoading: baseLaminationLoading,
    refetch: baseLaminationRefetch,
  ...
  useEffect(() => {
    if (
      ...
      baseLaminationData?.resultCode === "OK_0000"
    ) {
  ...
  >({
    ...
    queryFn: async () => {
      ...
      if (result.resultCode === "OK_0000") {
        const specLamination = result.data?.data?.specDetail?.data as {
          index: number;
          data: any;
          mappingResult: laminationSourceList;
        }[];
        let arr: laminationSourceList[] = [];
