...
import {
  productLinesGroupRType,
  productLinesRType,
} from "@/data/type/base/product";
...
const ProcessSelection: React.FC<Props> = ({
  ...
  const [selectPrdGrpTmp, setSelectPrdGrpTmp] =
    useState<productLinesGroupRType | null>(null);
      ...
      title: (
        <div className="parent-node flex items-center gap-10">
          ...
          <div className="flex-1 v-between-h-center">
            ...
            <span className="parent-count font-bold text-[#888]">
                  ...
                  <Radio.Group
                    value={
                      selectedVendors.find(
                        (sv) => String(sv.pid) === String(process.id)
                      )?.vid || ""
                    }
                  >
  ...
  const handleVendorSelect = (
    ...
    setSelectedVendors((prev) => {
      ...
      if (exists && exists.vid === vendorId) {
        ...
        return [
          ...prev.filter((item) => item.pid !== processId),
          { pid: processId, vid: vendorId, vname: vendorName },
        ];
      }
    });
  };
  ...
  const { data: prcQueryData } = useQuery<apiGetResponseType, Error>({
  ...
  const handleChangePrc = (processes?: processRType[]) => {
    ...
    if (processes) {
      processes?.map((item: processRType) => {
        arr.push(item.id);
        const vid = dataProcessGrp
          .find((f) => f.id === item.processGroup?.id)
          ?.processes.find((f) => f.id === item.id)
          ?.processVendors?.[0]?.vendor;
        if (vid)
          varr.push({
            pid: item.id,
            vid: vid?.id ?? "",
      ...
      if (result.resultCode === "OK_0000") {
        ...
        setDetailData({
          ...
          specPrdGroupPrcs: jsonData?.data?.map((item, index) => ({
            ...
            vendor: {
              id: item.vendorIdx,
              prtNm: selectedVendors.find((f) => f.vid === item.vendorIdx)
                ?.vname,
            },
          })),
        });
  ...
  return (
    <div className="w-full h-full h-center gap-10">
      <Popup className="!w-1/3 !h-[650px] !flex-col">
        ...
        <div className="w-full flex flex-col gap-10">
            ...
            <div className="flex-1">
              <AntdSelect
                ...
                onChange={(e) => {
                  ...
                  const rdata = prdGrpQueryData?.data
                    ?.data as productLinesGroupRType[];
                  ...
                  if (prc && prc.productLines) {
                    prc.productLines.map((item: productLinesRType) => {
                      if (item.process)
                        arr.push({ ...item.process, remark: item.prcWkRemark });
                    });
          ...
          <TreeStyled $cursor={view ? true : false}>
            {!dataLoading && (
              <Tree
                showIcon
                checkable
                ...
                onCheck={(_, info) => {
                  ...
                  if (info.checked) {
                    ...
                    const vendors = dataProcessGrp
                      .find((f) => f.id === addData?.processGroup?.id)
                      ?.processes.find((f) => f.id === id)?.processVendors;
                    if (addData) {
                      ...
                      if (vendors && vendors.length > 0) {
                        setSelectedVendors((prev) => [
                          ...
                          {
                            pid: addData.id,
                            vid: vendors?.[0].vendor?.id ?? "",
                            vname: vendors?.[0]?.vendor?.prtNm ?? "",
                          },
                        ]);
                      }
                    }
                  ...
                  if (info.children) {
                    ...
                    info.children.map((child: any) => {
                      ...
                      if (selectedKeys.includes(id)) {
                        ...
                        setSelectedKeys((prev: Array<string>) =>
                          prev.filter((key: string) => key !== id)
                        );
                        ...
                        const vendorCandidates = dataProcessGrp
                          .find(
                            (group) => group.id === addData?.processGroup?.id
                          )
                          ?.processes.find(
                            (proc) => proc.id === id
                          )?.processVendors;
              ...
              <div key={process.id} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <div className="flex-1 h-full h-center gap-50">
                    ...
                    <div
                      className="flex-1 h-full h-center gap-10 text-[#444444]"
                      style={{ letterSpacing: -0.05 }}
                    >
                      ...
                      {!view && (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setSelectPrc(
                              selectPrc.filter((p) => p.id !== process.id)
                            );
                          }}
                        >
                          <p className="w-14 h-14">
                            <Trash />
                          </p>
                        </div>
                      )}
      ...
      <AntdAlertModal
        ...
        contents={
          resultType === "already" ? (
            <div>
              현재 공정에 설정한 메모도 사라집니다.
              <br />
              정말 해당 제품군의 공정으로 변경하시겠습니까?
            </div>
          ) : resultType === "error" ? (
            <div>{errMsg}</div>
          ) : (
        ...
        okText={
          resultType === "already"
            ? "선택한 제품군의 공정으로 변경 할래요"
            : "확인"
        }
        cancelText={resultType === "already" ? "현재 공정으로 할래요" : "취소"}
        maskClosable={false}
      />
      <ToastContainer />
    </div>
