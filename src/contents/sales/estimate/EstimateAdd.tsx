...
const EstimateAddLayout = () => {
  ...
  // 베이스 값 가져오기
  const { metarialSelectList, unitSelectList } = useBase();
  ...
  useEffect(() => {
    ...
    if (prtId && prtId !== "" && cs?.data?.data && cs?.data?.data?.length) {
      const managers = ((cs?.data?.data as partnerRType[]) ?? []).find(
        (f) => f.id === prtId
      )?.managers;
    ...
    const updatedData = products.map((item) => {
      if (item.id === id) {
        ...
        if (name === "unitPrice") {
          ...
          const tot =
            (item.autoCalculatedUnitPrice && item.autoCalculatedUnitPrice > 0
              ? item.autoCalculatedUnitPrice
              : item.unitPrice ?? 0) * Number(value ?? 0);
          targetObject["cost"] = tot;
  ...
  const [modelDrawerSelectedKey, setModelDrawerSelectedKey] = useState<
    string | null
  >(null);
  ...
  const handleModelChange = (
    ...
    if (index > -1) {
      newData[index] = {
        ...newData[index],
        currPrdInfo:
          typeof newData[index].currPrdInfo === "string"
            ? newData[index].currPrdInfo
            : {
                ...JSON.parse(newData[index].currPrdInfo ?? "{}"),
                ...model,
              }.toString(),
          ...
          <div className="w-[calc(100%-100px)] h-full">
            ...
            <div className="w-full !h-[calc(100%-80px)] overflow-y-auto flex flex-col gap-20">
                  ...
                  <div className="w-[222px] flex flex-col gap-20">
                    ...
                    <LabelItem label="견적일">
                      <AntdDatePicker
                        ...
                        className="!w-full !rounded-2 !h-36 !border-[#D9D9D9]"
                        suffixIcon={"cal"}
                      />
                    </LabelItem>
              ...
              <div className="w-full v-between-h-center px-30">
                ...
                >
                  <CloseOutlined />{" "}
                  {!(formData?.id ?? "").includes("new") ? "삭제" : "취소"}
                </Button>
                {stepCurrent < 1 && (
                  <Button
                    ...
                    <div className="w-full">
                      <Popup className="overflow-auto !gap-20">
                        ...
                        <div className="v-between-h-center">
                          <span>
                            등록된 모델 중 선택 체크된 모델만 견적서에
                            포함됩니다.
                          </span>
                          <div className="flex-1 h-center justify-end gap-20 h-46">
                            <span>
                              선택된 모델의 합계 금액(총 견적 금액) :{" "}
                              {(
                                formData?.totalEstimatePrice ?? 0
                              ).toLocaleString()}
                              원
                            </span>
                            <span>
                              모델 총 합계 금액 : {totAll.toLocaleString()}원
                            </span>
                          </div>
                        </div>
