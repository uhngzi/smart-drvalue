...
import { HotGrade, ModelStatus, SalesOrderStatus } from "@/data/type/enum";
import {
...
const OrderAddLayout = () => {
  ...
  // 발주 저장 변수
  const [formData, setFormData] = useState<salesOrderCUType>(
    newDataSalesOrderCUType
  );
  // 발주 내 첨부파일
  ...
  const [newProducts, setNewProducts] = useState<salesOrderProcuctCUType[]>([
    { ...newDataSalesOrderProductCUType(), id: "new-1", index: 1 },
  ]);
  // 수정 시 필요 변수
  ...
  useEffect(() => {
    if (formData.partnerId !== "" && cs?.data?.data?.length) {
      ...
      const mng = data.find(
        (cu: partnerRType) => cu.id === formData.partnerId
      )?.managers;
      setCsMngList(mng ?? []);
      if (mng && mng.length > 0) {
  ...
  const fetchDetail = async (modelId?: string) => {
    ...
    if (result.resultCode === "OK_0000") {
      ...
      setNewProducts(
        prdArr.map((prd: salesOrderProductRType, index: number) => ({
          id: prd.id,
          index: index + 1,
          glbStatus: {
            ...prd.glbStatus,
          ...
          currPrdInfo:
            prd.currPrdInfo && typeof prd.currPrdInfo === "string"
              ? JSON.parse(prd.currPrdInfo)
              : {},
          ...
          orderPrdDueReqDt: prd.orderPrdDueReqDt
            ? dayjs(prd.orderPrdDueReqDt, "YYYY-MM-DD")
            : null,
          orderPrdDueDt: prd.orderPrdDueDt
            ? dayjs(prd.orderPrdDueDt, "YYYY-MM-DD")
            : null,
          orderPrdHotGrade: prd.orderPrdHotGrade ?? HotGrade.NORMAL,
          disabled:
            prd.glbStatus.salesOrderStatus ===
  ...
  const handleSubmitModel = async (model: salesOrderProcuctCUType) => {
    try {
      const jsonData = {
        ...
        partnerManagerId:
          formData.partnerManagerId === "" ? null : formData.partnerManagerId,
        ...
        orderPrdDueDt: model.orderPrdDueDt
              ...
              setTimeout(
                () =>
                  handleConfirm(
                    model.id ?? "",
                    model.modelId ?? "",
                    model.modelStatus ?? ModelStatus.REPEAT
                  ),
                500
              );
            }
  ...
  const handleEditOrderMain = async (auto?: boolean) => {
    const jsonData = {
      ...
      orderDt: formData.orderDt
        ? dayjs(formData.orderDt).format("YYYY-MM-DD")
    ...
    <>
      <div className="px-30 min-h-70 !h-70 v-between-h-center w-full">
        <p className="text-18 font-[500]">
          {id?.includes("new") ? "고객 발주 등록" : "고객 발주 수정"}
        </p>
        <p
                  ...
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (!orderVal.isValid) {
                          showToast(
                            orderVal.missingLabels + "은(는) 필수 입력입니다.",
                            "error"
                          );
                        } else {
                          handleAddOrderMain();
          ...
          <AntdAlertModal
            ...
            title={
              alertType === "del"
                ...
                ? "해당 발주를 폐기하시겠습니까?"
                : alertType === "close"
                ? "저장되지 않은 모델 정보 또는 미등록된 고객 발주 정보가 있습니다."
                : alertType === "error"
                ? "오류 발생"
                : alertType === "discard"
                ? "고객 발주 폐기 성공"
            ...
            contents={
              alertType === "del" ? (
                <div>
                  이미 등록된 모델을 삭제하실 경우 모델 등록 대기에서도
                  사라집니다.
                  <br />
                  정말 삭제하시겠습니까?
                </div>
                ...
                <>
                  해당 발주를 폐기할 경우 하위에 등록된 모델도 사라집니다.
                  <br />
                  정말 폐기하시겠습니까?
                </>
              ) : alertType === "close" ? (
                <>
                  저장하지 않고 나가시면 해당 정보는 삭제됩니다.
                  <br />
                  정말 나가시겠습니까?
                </>
              ) : alertType === "error" ? (
            ...
            hideCancel={
              alertType === "discard" || alertType === "error" ? true : false
            }
            okText={
              alertType === "del"
                ? "네 삭제할래요"
