...
const ModelAddLayout = () => {
  ...
  useEffect(() => {
    if (
      (model?.prdNm && model.prdNm.length > 2) ||
  ...
  const { refetch } = useQuery<apiAuthResponseType, Error>({
    ...
    queryFn: async () => {
      const result = await getAPI(
        ...
        },
        {
          ...
          s_query: {
            $or: [
              (model?.prdNm ?? "").length > 0
                ? { prdNm: { $startsL: model?.prdNm } }
                : {},
              (model?.prdMngNo ?? "").length > 0
                ? { prdMngNo: { $startsL: model?.prdMngNo } }
                : {},
            ],
          },
        }
    ...
    enabled:
      searchFlag &&
      ((model?.prdNm ?? "").length > 2 || (model?.prdMngNo ?? "").length > 2),
  });
  // ------------- 모델 데이터 세팅 ------------ 끝
  ...
  const handleModelDataChange = (name: string, value: any) => {
  ...
  return (
    <>
                    ...
                    <div className="h-center gap-15">
                      <div className="h-center gap-5">
                        ...
                        <AntdInput
                          ...
                          onChange={(e) =>
                            handleModelDataChange("orderPrtNo", e.target.value)
                          }
                      ...
                      <div className="h-center gap-5 flex-1">
                        ...
                        <AntdInput
                          value={model?.remarks}
                          onChange={(e) =>
                            handleModelDataChange("remarks", e.target.value)
                          }
                          styles={{ ht: "32px", bg: "#FFF" }}
                          memoView
                          readonly={model?.usedYn}
                ...
                >
                  <Button
                    ...
                    style={
                      model?.usedYn
                        ? {}
                        : { color: "#ffffffE0", backgroundColor: "#4880FF" }
                    }
      ...
      <AntdAlertModal
        ...
        cancelText={alertType === "close" ? "아니요 이어서 작성할게요" : ""}
      />
